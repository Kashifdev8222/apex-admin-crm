  ...
  return (
    <div className="kpi-grid">
      <div className="kpi-card">
        ...
        <div className="kpi-meta muted">{s.tenants} tenant{s.tenants === 1 ? "" : "s"}</div>
      </div>
      <div className="kpi-card">
      ...
      <div className="kpi-card">
        ...
        <div className="kpi-meta warn">{fmtCompact(s.pendingDepositAmount)} awaiting</div>
      </div>
      <div className="kpi-card">
        <div className="kpi-lbl">Pending withdrawals</div>
        <div className="kpi-val">{s.pendingWithdraws}</div>
        <div className="kpi-meta bad">{fmtCompact(s.pendingWithdrawAmount)} awaiting</div>
      </div>
    </div>
  );
}
...
async function DashboardPanels() {
  ...
  const approvals: ApprovalRow[] = [
    ...txs.map((t) => ({
      ...
      amountLabel:
        (t.type === "DEPOSIT" ? "+" : "-") + money(Number(t.amount), t.currency),
    ...
    <div className="grid-2">
      ...
      <div className="dash-panel">
          ...
          recentTx.map((t) => {
            ...
            return (
              <div className="activity-row" key={t.id}>
                <span className={`badge ${isDep ? "badge-blue" : "badge-amber"}`}>
                  {capitalize(t.type)}
                </span>
        ...
        <div style={{ marginTop: 16 }}>
          <Link href="/transactions" style={{ fontSize: 12, color: "#024c8d", fontWeight: 500 }}>
